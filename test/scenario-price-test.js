...
const mkParams = mode => ({
  ...PARAMS,
  wholesaleCurves: [],   // 用默认假设路径测 direct/标定差异（避免内置采购曲线干扰缺口）
...
ok('direct 模式 calibK=1', rD.scenarios[0].calibK === 1);
...
ok('direct/标定 Cda 比例 ≈ 直接均价/W 均价（≈0.8）', ratio > 0.75 && ratio < 0.85, ratio.toFixed(4));
ok('direct Cda < 标定 Cda（400 < 500）', cdaD < rC.scenarios[0].Cda);
